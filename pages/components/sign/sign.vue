	...
	<view class="sign-container">
		...
		<template v-if="isHost">
			<view class="host-view">
				<view class="header">
					<text class="title">{{ hasStartedSign ? '签到情况' : '发起签到' }}</text>
				</view>
					...
					<view class="sign-form">
						<view class="form-item">
							...
							<view class="duration-picker">
								<text class="duration" @click="showDurationPicker">{{ duration }}分钟</text>
				...
				<view class="sign-list" :class="{ 'full-height': hasStartedSign }">
					<view class="list-header">
						...
						<text class="sign-count">已签到 {{ signedCount }}/{{ totalCount }}</text>
					</view>
					<view class="list-content">
						<view v-for="(item, index) in signList" :key="index" class="sign-item">
							...
							<view class="sign-status" :class="{ signed: item.signInStatus === 1 }">
		...
		<template v-else>
			<view class="attendee-view">
				<view class="header">
					...
					<text class="subtitle">{{ hasSignedIn ? '签到完成' : '主办方发起了签到' }}</text>
				</view>
				
				<view class="timer-section">
					<view class="timer-wrapper">
						<view v-if="!hasSignedIn" class="timer-circle" :style="{ background: `conic-gradient(#2B58F9 ${progressDegree}deg, #E8EFFF ${progressDegree}deg)` }">
...
const getSignStatus = async () => {
	try {
		const response = await uni.request({
			url: `http://192.168.31.115:5000/meetingSign?meetingId=${meetingId.value}`,
			method: 'GET'
		});
		const result = response.data;
		console.log('获取签到情况',result);
		if (result.code === 1) {
			signList.value = result.data;
			// 检查当前用户是否已签到
			const userRecord = signList.value.find(item => item.participantUid === userId.value);
			if (userRecord) {
...
const submitAbsence = async () => {
	...
	try {
		const response = await uni.request({
			url: 'http://192.168.31.115:5000/meetingSign/updateAbsence',
			method: 'POST',
			data: {
				meetingId: meetingId.value,
				participantUid: userId.value,
...
onLoad((options) => {
	...
	console.log(signMethod.value);
	

	if (!isHost.value) {
		totalTime.value = duration.value * 60;
