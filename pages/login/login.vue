<template>
	<view class="login-container">
		...
		<!-- 主要内容区 -->
		<scroll-view scroll-y class="content-wrapper" :scroll-top="scrollTop" :style="{ height: windowHeight + 'px' }">
			<!-- Logo区域 -->
			<view class="logo-area" :class="{ 'fade-in-down': showLoginBox, 'logo-small': isKeyboardOpen }">
				<image class="app-logo" src="/src/static/images/app.jpg" mode="aspectFill"></image>
				<text class="app-name">智慧会议</text>
				<text class="app-slogan" v-if="!isKeyboardOpen">让会议更高效，沟通更便捷</text>
			</view>
			
			<!-- 登录框 -->
			<view class="login-box" :class="{ 'fade-in-up': showLoginBox, 'login-box-keyboard-open': isKeyboardOpen }">
				...
				<view class="input-group">
					<view class="input-item" :class="{ 'active': activeInput === 'username' }">
					...
					<view class="input-item" :class="{ 'active': activeInput === 'password' }">
			...
			<view class="keyboard-space" v-if="isKeyboardOpen" :style="{ height: keyboardHeight + 'px' }"></view>
			
			<view class="footer-text" :class="{ 'fade-in': showLoginBox, 'footer-hidden': isKeyboardOpen }">
				<text>智慧会议 © 2025</text>
			</view>
		</scroll-view>
	</view>
