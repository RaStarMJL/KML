...
const imagePreview_avatar = ref("");

const imagePreview_background = ref("");
...
const changeBackground = async () => {
  try {
    ...
    imagePreview_background.value = res.tempFilePaths?.[0] || "";
    console.log(imagePreview_background.value);

    // 发送请求
    const response = await uni.uploadFile({
      url: baseURL + "user/updateBackUrl",
      filePath: imagePreview_background.value,
      name: "file",
      formData: {
        userId: uid,
      },
    ...
    if (result.code === 1) {
      // 更新本地显示的背景图片
      userInfo.value.backgroundUrl = imagePreview_background.value;
      userInfoStore.setProfile({
        ...userInfoStore.userInfo,
        backgroundUrl: imagePreview_background.value,
      });
      uni.hideLoading();
      uni.showToast({
        title: "背景上传成功",
...
const changeAvatar = async () => {
  try {
    ...
    imagePreview_avatar.value = res.tempFilePaths?.[0] || "";
    console.log(imagePreview_avatar.value);
    if (!res.tempFilePaths?.[0]) {
      throw new Error("未选择图片");
    }
    ...
    // 发送请求
    const response = await uni.uploadFile({
      url: baseURL + "user/updateAvatarUrl",
      filePath: imagePreview_avatar.value,
      name: "file",
      formData: {
        userId: uid,
      },
    ...
    if (result.code === 1) {
      // 更新本地显示的头像
      userInfo.value.avatarUrl = imagePreview_avatar.value;
      userInfoStore.setProfile({
        ...userInfoStore.userInfo,
        avatarUrl: imagePreview_avatar.value,
      });
      uni.showToast({
        title: "头像上传成功",
        icon: "success",
...
const getUserInfo = async () => {
  try {
    ...
    userInfo.value = {
      ...
      backgroundUrl:
        storeUserInfo.backgroundUrl || "/src/static/images/default-bg.jpg",
      avatarUrl:
        storeUserInfo.avatarUrl || "/src/static/images/defaultAvatar.png",
    };
