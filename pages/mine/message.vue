<template>
	<view class="message-container">
		...
		<scroll-view scroll-y class="message-list">
			...
			>
				<view class="item-main" :style="{ transform: `translateX(${currentOpenMessageId === message.id ? offset : 0}rpx)` }">
					...
					<view class="message-content">
						...
						<view class="message-tag meeting-remind" v-if="message.messageType === '会议提醒'">
							<text>点击进入</text>
						</view>
					</view>
				</view>
				<view class="delete-btn-container">	
					<view class="delete-btn read" @click.stop="markAsRead(message)">标记已读</view>
					<view class="delete-btn" @click.stop="deleteMessage(index)">删除</view>
				</view>
			</view>
...
export default {
	...
	methods: {
				...
				if (res.code === 1) {
					...
					res.data.forEach(msg => {
						const processedMsg = {
							...
							type: msg.messageType === '会议签到' || msg.messageType === '会议提醒' ? 'meeting' : 'system'
						};
						...
						// 根据类型添加到对应的分类数组
						if (msg.messageType === '会议签到' || msg.messageType === '会议提醒') {
		...
		async handleMessageClick(message) {
			...
			if	(message.messageType === '会议签到'){
				...
				uni.navigateTo({
					url: '/pages/sign/sign?role=attendee&meetingId=' + message.meetingUid + '&meetingname=' + res.data.meetingName
				});
    ...
    try {
        ...
        const res = await uni.request({
            url: 'http://192.168.31.115:5000/PushNotification/deleteUserMessageReceive?id=' + id,
            method: 'DELETE',
        });
		...
		touchStart(event, messageId) {
			if (this.currentOpenMessageId && this.currentOpenMessageId !== messageId) {
				this.offset = 0;
				this.currentOpenMessageId = null;
			}
