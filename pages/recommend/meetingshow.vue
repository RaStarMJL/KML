	...
	<view class="meeting-detail">
			...
			<view class="basic-info">
				<view class="info-row">
					...
					<view class="time-info">
						...
						<text class="time">{{ formatTime(meetingInfo.startTime) }} - {{ formatTime(meetingInfo.endTime) }}</text>
					</view>
				</view>
				<view class="info-row">
		...
		<view class="section">
			...
			<view class="attendees-list">
				<view class="attendee-item" v-for="(uid, index) in meetingInfo.attendeesUid" :key="index">
					<image class="attendee-avatar" :src="'/static/avatars/' + uid + '.jpg'" mode="aspectFill"/>
					<text class="attendee-name">用户{{ uid }}</text>
				</view>
				<view class="attendee-item add-more" v-if="meetingInfo.numAttendees > 6">
		...
		<view class="bottom-buttons">
			<button class="btn join-btn" @click="joinMeeting">参加会议</button>
			...
			async fetchMeetingInfo() {
				try {
					...
					const response = await uni.request({
						url: `http://192.168.31.115:5000/meetMessage?meetId=${this.meetingId}`,
						method: 'GET'
					})
